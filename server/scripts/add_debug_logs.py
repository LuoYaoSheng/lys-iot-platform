...
def add_debug_logs():
    ...
    for i, line in enumerate(lines):
        ...
        if 'if req.Service != nil && *req.Service != ""' in line:
            insert_positions.append((i+1, '\t\tfmt.Printf("[DEBUG] Entering thing model service branch, Service=%v, Params=%v\\n", req.Service, req.Params)\\n'))

        # 在 "params = req.Params" 前添加日志
        if 'params = req.Params' in line and '_:=' not in line:
            insert_positions.append((i, '\t\tfmt.Printf("[DEBUG] Before params assignment, req.Params=%v, len=%d\\n", req.Params, len(req.Params))\\n'))

        # 在 "if len(params) == 0" 前添加日志
        if 'if len(params) == 0' in line:
            insert_positions.append((i, '\tfmt.Printf("[DEBUG] Final params check, params=%v, len=%d\\n", params, len(params))\\n'))
